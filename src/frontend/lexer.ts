...
export const TimeLiteral = createToken({
  name: 'TimeLiteral',
  pattern: /(?:T|TIME)#[0-9_]+(?:d|h|m|s|ms|us|ns)?(?:[0-9_]+(?:d|h|m|s|ms|us|ns)?)*(?:\.[0-9_]+)?/i,
});
...
export const DateTimeLiteral = createToken({
  ...
  pattern: /(?:DT|DATE_AND_TIME)#[0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]+)?/i,
});
...
export const OutputAssign = createToken({ name: 'OutputAssign', pattern: /=>/ });
