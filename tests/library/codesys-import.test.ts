...
describe("V3 integration: OSCAT Basic 335", () => {
  ...
  it("auto-extracts inline documentation blocks from V3 sources", () => {
    // OSCAT POUs all carry a structured `(* version X.Y / programmer /
    // tested by / description … *)` block right after their VAR
    // sections. The library compiler picks this up by regex (during
    // compileStlib) and tags each manifest entry's `documentation`
    // field with the trimmed block body. This means library.json no
    // longer needs a hand-curated `blocks: {…}` doc map for codesys-
    // imported libs — the .library file is self-documenting.
    //
    // We can't observe `documentation` on ExtractedPOU from the
    // import alone (it lives at the manifest level, set during
    // compile). So we go through the full rebuild output: the OSCAT
    // .stlib is regenerated by vitest globalSetup before this test
    // runs. We assert every OSCAT POU's manifest entry has a
    // non-empty documentation string.
    ...
    const allEntries: Array<{ name: string; documentation?: string }> = [
      ...archive.manifest.functions,
      ...archive.manifest.functionBlocks,
    ];
    const docCount = allEntries.filter((e) => e.documentation).length;
    expect(allEntries.length).toBeGreaterThan(500); // OSCAT has 545 entries
    // Every POU's source carries a doc block — coverage must be 100%
    // for the codesys-derived metadata claim to hold.
    expect(docCount).toBe(allEntries.length);

    // Spot-check a few well-known POUs to pin the parsing's output
    // shape: the leading "version X.Y\tdate" header line should be
    // preserved verbatim.
    const dcf = archive.manifest.functionBlocks.find(
      (fb: { name: string }) => fb.name === "DCF77",
    );
    expect(dcf?.documentation).toMatch(/version\s+1\.10/);
    expect(dcf?.documentation).toMatch(/decoder for a DCF77 signal/);

    const acosh = archive.manifest.functions.find(
      (f: { name: string }) => f.name === "ACOSH",
    );
    expect(acosh?.documentation).toMatch(/arcus cos hyperbolicus/);
  });
